...
public class SemanticAnalyzer implements ASTVisitor {
    ...
    /**
     * Thực thi quy trình phân tích ngữ nghĩa theo đúng thứ tự 4 bước tiêu chuẩn:
     * 1. NameResolver.resolve(ast)
     * 2. TypeChecker.validate(ast)
     * 3. GroupByValidator.validate(ast)
     * 4. OrderByValidator.validate(ast)
     *
    ...
    public void analyze(AST ast) {
        ...
        // 2. Thẩm định tính tương thích kiểu dữ liệu trong biểu thức
        typeChecker.validate(ast);

        // 3. Kiểm tra điều kiện ràng buộc mệnh đề GROUP BY
        groupByValidator.validate(ast);

        // 4. Kiểm tra danh sách sắp xếp mệnh đề ORDER BY
